...
class S3FileUpload {
    static async uploadFile(file, config, fileConfig) {

        // Error Thrower :x:
        throwError(config, file, fileConfig)

        const fd = new FormData();
        const key = `${config.dirName ? config.dirName + "/" : ""}${fileConfig.name}`;
        const url = `https://${config.bucketName}.s3.amazonaws.com/`;
        fd.append("key", key);
        fd.append("acl", "public-read");
        fd.append("Content-Type", fileConfig.type);
